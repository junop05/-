...
import { Users, Activity, Plus, Trophy, X, Shirt, Calendar, Camera, Trash2, PlayCircle, Settings, ClipboardList, RefreshCw, BarChart3, FastForward, ArrowLeft, Lock, Image as ImageIcon, ZoomIn, ZoomOut, Save, ChevronDown, ChevronUp, MoreVertical, Edit, Undo, Download, Award } from 'lucide-react';
...
function parseBaseballInningsToOuts(inningsValue) {
...
function calculateEra(earnedRuns, inningsOrOuts) {
...
function calculateFip(stats, leagueEra = 3.10) {
  const outs = parseBaseballInningsToOuts(stats?.innings || 0);
  const ip = outs / 3;
  if (ip <= 0) return '0.00';
  const hr = Number(stats?.homeRunsAllowed || 0);
  const bb = Number(stats?.walksAllowed || 0);
  const hbp = Number(stats?.hitByPitch || 0);
  const wp = Number(stats?.wildPitches || 0);
  const k = Number(stats?.strikeouts || 0);
  const C = Number(leagueEra) > 0 ? Number(leagueEra) : 3.10;
  const fip = (13 * hr + 3 * (bb + hbp) + 2 * wp - 2 * k) / ip + C;
...
function RadarChart({ data, color }) {
  ...
  const getPoint = (value, index, max = 100) => {
  ...
  const maxValues = data.map(d => Number(d.max) || 100);

  const polygonPoints = values.map((val, i) => getPoint(val, i, maxValues[i])).join(' ');
  const bgPolygons = [100, 80, 60, 40, 20].map(level => 
    data.map((_, i) => getPoint(level, i, 100)).join(' ')
  );

  return (
    <svg width="100%" height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible font-sans">
      {bgPolygons.map((pts, idx) => (
        <polygon key={`bg-poly-${idx}`} points={pts} fill="none" stroke="#e2e8f0" strokeWidth="1" />
      ))}
      {data.map((_, i) => {
        const endPt = getPoint(100, i, 100);
        const [ex, ey] = endPt.split(',');
        return <line key={`axis-${i}`} x1={center} y1={center} x2={ex} y2={ey} stroke="#e2e8f0" strokeWidth="1" />;
      })}
      <polygon points={polygonPoints} fill={color} fillOpacity="0.4" stroke={color} strokeWidth="2" strokeLinejoin="round" />
      {values.map((val, i) => {
        const pt = getPoint(val, i, maxValues[i]);
        const [cx, cy] = pt.split(',');
        return <circle key={`dot-${i}`} cx={cx} cy={cy} r="3" fill={color} />;
      })}
      {labels.map((label, i) => {
        const pt = getPoint(115, i, 100);
        const [x, y] = pt.split(',');
        return (
          <text key={`label-${i}`} x={x} y={y} fontSize="10" fill="#64748b" textAnchor="middle" dominantBaseline="middle" fontWeight="bold">
            {label}
          </text>
        );
      })}
...
export default function App() {
  ...
  const [recordYear, setRecordYear] = useState('all'); // 👈 이 줄 추가
  ...
  const [scheduleForm, setScheduleForm] = useState({ title: '', date: '', type: '훈련', location: '' });
  const [showRsvpModal, setShowRsvpModal] = useState(null);
  const [rsvpPlayerId, setRsvpPlayerId] = useState('');
  const [rsvpStatus, setRsvpStatus] = useState('참석');
  ...
  useEffect(() => {
    ...
    const unsubGames = onSnapshot(collection(db, 'polaris_games'), (snap) => {
      const loaded = snap.docs.map(d => d.data());
      setGameResults(loaded.sort((a,b) => b.id - a.id));
    });

    const unsubSchedules = onSnapshot(collection(db, 'polaris_schedules'), (snap) => {
      const loaded = snap.docs.map(d => d.data());
      setSchedules(loaded.sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime()));
    });

    const unsubGallery = onSnapshot(collection(db, 'polaris_gallery'), (snap) => {
      const loaded = snap.docs.map(d => d.data());
      setGalleryPosts(loaded.sort((a,b) => b.id - a.id));
    });

    const unsubPhotos = onSnapshot(collection(db, 'polaris_playerPhotos'), (snap) => {
      const loaded = snap.docs.map(d => d.data());
      const photoMap = loaded.reduce((acc, item) => {
...
const allPlayers = useMemo(() => players, [players]);
  ...
  // 폴라리스 팀 전체 평균 ERA = FIP 상수로 사용
  const teamAvgEra = useMemo(() => {
    let totalEr = 0;
    let totalOuts = 0;
    players.forEach((p) => {
      if (p?.pitching) {
        totalEr += Number(p.pitching.earnedRuns || 0);
        totalOuts += parseBaseballInningsToOuts(p.pitching.innings || 0);
      }
    });
    if (totalOuts <= 0) return 3.10;
    const era = (totalEr * 9) / (totalOuts / 3);
    return Number(era.toFixed(2));
  }, [players]);
  ...
  const handleAddRecord = async (e) => {
    ...
    if (['타자', '투타겸업'].includes(playerRole)) {
      ...
      newPlayer.batting.avg = String(calculateBattingAverage(hits, atBats));
    } 
    ...
    await setDoc(doc(db, 'polaris_players', newPlayer.id.toString()), newPlayer);
  ...
  const handleDeletePlayer = async (id) => {
    ...
    if (window.confirm('정말로 이 선수를 삭제하시겠습니까?')) {
      await deleteDoc(doc(db, 'polaris_players', id.toString()));
    }
  };
  ...
  const handlePositionChange = async (id, newPosition) => {
    ...
    if (player) {
      await setDoc(doc(db, 'polaris_players', id.toString()), { ...player, position: newPosition });
    }
  };
  ...
  const handleSaveCareer = async (e) => {
    e.preventDefault();
    if (!user) return;
    const p = players.find(p => p.id === careerPlayer.id);
    ...
    if (p) {
      ...
      const p_ip = String(careerForm.p_innings || '0');
      const p_era = String(calculateEra(p_er, p_ip));
      ...
      const updatedPlayer = {
        ...
        pitching: {
          ...
          career: {
            ...
            fip: calculateFip({
              innings: p_ip,
              homeRunsAllowed: parseInt(careerForm.p_homeRunsAllowed) || 0,
              walksAllowed: parseInt(careerForm.p_walksAllowed) || 0,
              hitByPitch: parseInt(careerForm.p_hitByPitch) || 0,
              wildPitches: parseInt(careerForm.p_wildPitches) || 0,
              strikeouts: parseInt(careerForm.p_strikeouts) || 0
            }, teamAvgEra)
          }
        }
      ...
      await setDoc(doc(db, 'polaris_players', p.id.toString()), updatedPlayer);
    }
  ...
  const handleAddSchedule = async (e) => {
    ...
    await setDoc(doc(db, 'polaris_schedules', newId), newSchedule);
    setShowScheduleModal(false);
    setScheduleForm({ title: '', date: '', type: '훈련', location: '' });
  };

  const handleDeleteSchedule = async (id) => {
    if (!user) return;
    if (window.confirm("일정을 삭제하시겠습니까?")) {
      await deleteDoc(doc(db, 'polaris_schedules', id.toString()));
    }
  };
  ...
  const handleRsvpSubmit = async (e) => {
    e.preventDefault();
    if (!user) return;
    if (!rsvpPlayerId) return alert("선수를 선택해주세요.");
  ...
  const executeWithHistory = (actionFn) => {
    setGameStateHistory(prev => [...prev.slice(-19), JSON.parse(JSON.stringify(gameState))]);
    actionFn();
  };
 ...
 const startRegularSetup = () => {
  ...
  const startGame = () => {
    ...
    for (const team of ['teamA', 'teamB']) {
      ...
      const pitcher = players.find(p => String(p.id) === String(pitcherRawId));
...
/* [PATCH] pitcher-in-lineup */
  ...
  const requestPitcherChange = (teamKey, newPitcherIdRaw) => {
    ...
    const dhSlotIndex = (team.lineup || []).findIndex(
      p => (p.assignedPosition === '지명타자' || p.position === '지명타자')
    );
    const newPitcherInLineupIdx = (team.lineup || []).findIndex(
      p => String(p.id) === String(newPitcherIdRaw)
    );

    // 라인업에 있는 야수를 투수로 올리고 + 기존 DH가 존재하면 → 소멸 여부 확인
    if (newPitcherInLineupIdx >= 0 && dhSlotIndex >= 0 && newPitcherInLineupIdx !== dhSlotIndex) {
      setPitcherSwapAsk({ teamKey, newPitcherId: newPitcherIdRaw, dhSlotIndex });
      return;
    }
    executePitcherChange(teamKey, newPitcherIdRaw);
  };
  ...
  const executePitcherChange = (teamKey, newPitcherIdRaw) => {
    executeWithHistory(() => {
      ...
      const newPitcher = players.find(p => String(p.id) === String(newPitcherIdRaw));
      if (newPitcher) {
        setGameState(prev => {
  ...
  const openLineupChange = (teamKey) => {
    ...
    setLineupChangeTeam(teamKey);
  };

  const updateLineupDraft = (idx, field, value) => {
    setLineupDraft((prev) => {
      const next = [...prev];
    ...
    executeWithHistory(() => {
      setGameState((prev) => {
        ...
        const newLineup = lineupDraft.map((slot, idx) => {
          ...
          if (oldId === newId) {
            return { ...oldSlot, assignedPosition: slot.assignedPosition || oldSlot.assignedPosition };
          }
        ...
        let nextBatterIndex = state[lineupChangeTeam].batterIndex || 0;
        if (newLineup.length > 0) {
          nextBatterIndex = nextBatterIndex % newLineup.length;
        }
        ...
        return {
          ...
          logs: newLogs,
          [lineupChangeTeam]: {
            ...state[lineupChangeTeam],
            lineup: newLineup,
            batterIndex: nextBatterIndex,
            substitutedOut: subbedOut
          }
        };
      });
    });
    setLineupChangeTeam(null);
    setLineupDraft([]);
  };
    ...
    executeWithHistory(() => {
      setGameState(prev => {
        ...
        if (isErrorPlay) {
          ...
          if (actionLabel === '볼넷' || actionLabel === '사구' || isDroppedThirdStrike) {
        ...
        let nextSummary = {
          ...
          [battingTeam]: {
            ...
            strikeouts: (state.summary[battingTeam]?.strikeouts || 0) + ((actionLabel === '삼진' || actionLabel === '낫아웃 출루') ? 1 : 0),
  ...
  const handleSacAction = (actionLabel) => {
    ...
    if (!hasRunners) {
      // 주자 없으면 그냥 아웃 처리
      executeSacResolution(actionLabel, []);
      return;
    }
    const runners = gameState.bases.map((r, i) => r ? {
      ...
      name: r.name,
      runnerObj: r,
      // 기본값: 희생플라이는 3루 주자 홈인, 그 외 한 베이스 진루
      choice: (actionLabel === '희생플라이' && i === 2) ? 4 : Math.min(i + 2, 4)
    } : null).filter(Boolean);
    setPendingSac({ actionLabel, runners });
  };

  const executeSacResolution = (actionLabel, runnerChoices) => {
    executeWithHistory(() => {
      setGameState(prev => {
        let state = normalizeGameStateForTracking(prev);
        const isTop = state.half === 'top';
        ...
        runnerChoices.forEach(rc => {
          ...
          if (choice === 0) {
            ...
            if (newBases[choice - 1]) {
              newLogs.unshift(`[경고] ${choice}루 주자 충돌`);
            }
            newBases[choice - 1] = rc.runnerObj;
          }
        });

        // 사용자가 선택한 추가 아웃을 totalOuts에 합산하지 않고 별도로 더함
        newOuts += extraOuts;
        pitchingDelta.inningsOuts += extraOuts;
...
const confirmEndGame = () => {
  ...
  const getBatterStats = (p, type) => {
    let statsData = p.batting || {};
    if (type === 'career') statsData = p.batting?.career || {};
    else if (type === 'season' && recordYear !== 'all') {
      statsData = p.yearlyStats?.[recordYear]?.batting || createBaseBatting();
    }
  ...
  const getPitcherStats = (p, type, teamAvgEra = 3.10) => {
    let statsData = p.pitching || {};
    if (type === 'career') statsData = p.pitching?.career || {};
    else if (type === 'season' && recordYear !== 'all') {
      statsData = p.yearlyStats?.[recordYear]?.pitching || createBasePitching();
    }
    ...
    return {
      ...stats, id: p.id, name: String(p.name || ''), uniformNumber: p.uniformNumber,
      whip: String(whip.toFixed(2)), era: String(calculateEra(stats.earnedRuns || 0, outs)),
      fip: String(calculateFip(stats, teamAvgEra))
    };
  };
  ...
  const renderLanding = () => (
    <div className="relative w-full h-screen flex flex-col justify-center items-center bg-black overflow-hidden">
      ...
      <div className={`absolute inset-0 z-0 bg-black overflow-hidden flex items-center justify-center transition-opacity duration-500 ${landingBgLoaded ? 'opacity-100' : 'opacity-0'}`}>
        <img 
          src={customBackground || DEFAULT_BG} 
          alt="팀 배경 사진" 
      ...
      <div className="relative z-10 flex-grow flex flex-col justify-center items-center text-center w-full px-4 pt-10 pointer-events-none">
        ...
        >
          <span className={`block text-5xl md:text-7xl lg:text-[6rem] tracking-widest mb-2 md:mb-4 text-gray-100 ${landingAnimReady ? 'animate-slide-left' : 'opacity-0'}`}>순천향의대</span>
          <span className={`block text-5xl md:text-7xl lg:text-[6rem] tracking-widest text-gray-100 ${landingAnimReady ? 'animate-slide-right' : 'opacity-0'}`}>폴라리스</span>
        </h1>
      ...
      {schedules.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {schedules.map((schedule, idx) => {
            ...
            return (
              <div key={`schedule-${schedule.id || idx}`} className={`bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col ${isPast ? 'opacity-60' : ''}`}>
                <div className={`p-4 border-b ${schedule.type === '경기' ? 'bg-blue-50 border-blue-100' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${schedule.type === '경기' ? 'bg-blue-600 text-white' : 'bg-slate-600 text-white'}`}>{schedule.type}</span>
                    {isAdminAuth && (
                      <button onClick={() => handleDeleteSchedule(schedule.id)} className="text-gray-400 hover:text-red-500"><Trash2 size={16} /></button>
                    )}
                  </div>
                  <h3 className="font-black text-xl text-gray-800 line-clamp-1">{schedule.title}</h3>
                  <p className="text-sm font-semibold text-gray-600 mt-1">{schedule.date} · {schedule.location}</p>
                </div>
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div className="flex gap-4 mb-5 text-sm">
                    <div className="bg-emerald-50 text-emerald-700 px-3 py-2 rounded-lg flex-1 text-center font-bold">참석: {attendCount}명</div>
                    <div className="bg-red-50 text-red-700 px-3 py-2 rounded-lg flex-1 text-center font-bold">불참: {absentCount}명</div>
                  </div>
                  <button 
                    onClick={() => setShowRsvpModal(schedule.id)} 
                    className="w-full bg-slate-800 hover:bg-black text-white font-bold py-3 rounded-xl transition-colors shadow-sm"
                    disabled={isPast}
                  >
                    {isPast ? '종료된 일정' : '참석 여부 투표하기'}
                  </button>
                </div>
              </div>
            );
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gray-50">
              <h3 className="font-bold text-gray-800">새 일정 추가</h3>
            ...
            <form onSubmit={handleAddSchedule} className="p-5 space-y-4">
              <div><label className="block text-xs font-bold text-gray-600 mb-1">일정명</label><input required type="text" name="title" value={scheduleForm.title} onChange={handleScheduleInputChange} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-slate-800 outline-none text-sm" placeholder="예: 토요 정기 훈련" /></div>
              <div><label className="block text-xs font-bold text-gray-600 mb-1">날짜</label><input required type="date" name="date" value={scheduleForm.date} onChange={handleScheduleInputChange} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-slate-800 outline-none text-sm" /></div>
              <div><label className="block text-xs font-bold text-gray-600 mb-1">장소</label><input required type="text" name="location" value={scheduleForm.location} onChange={handleScheduleInputChange} className="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-slate-800 outline-none text-sm" placeholder="예: 순천향대학교 운동장" /></div>
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-1">종류</label>
                <select name="type" value={scheduleForm.type} onChange={handleScheduleInputChange} className="w-full p-2.5 border rounded-lg outline-none bg-white text-sm">
                  <option value="훈련">훈련</option>
                  <option value="경기">경기</option>
                </select>
              </div>
              <div className="pt-2"><button type="submit" className="w-full bg-slate-800 text-white font-bold py-3 rounded-xl hover:bg-black transition-colors">일정 저장</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {showRsvpModal !== null && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gray-50">
              <h3 className="font-bold text-gray-800">참석 여부 투표</h3>
              <button onClick={() => setShowRsvpModal(null)} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
            </div>
            <form onSubmit={handleRsvpSubmit} className="p-5 space-y-5">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">누구신가요?</label>
                <select required value={rsvpPlayerId} onChange={(e) => setRsvpPlayerId(e.target.value)} className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-slate-800 bg-white">
                  <option value="">선수 선택...</option>
                  {players.map((p, idx) => <option key={`rsvp-${p.id || idx}`} value={p.id}>{p.name} (No.{p.uniformNumber})</option>)}
                </select>
              </div>
              <div className="flex gap-3">
                <label className={`flex-1 border-2 rounded-xl p-3 text-center cursor-pointer font-bold transition-all ${rsvpStatus === '참석' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                  <input type="radio" name="status" value="참석" className="hidden" checked={rsvpStatus === '참석'} onChange={() => setRsvpStatus('참석')} />
                  참석
                </label>
                <label className={`flex-1 border-2 rounded-xl p-3 text-center cursor-pointer font-bold transition-all ${rsvpStatus === '불참' ? 'border-red-500 bg-red-50 text-red-700' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                  <input type="radio" name="status" value="불참" className="hidden" checked={rsvpStatus === '불참'} onChange={() => setRsvpStatus('불참')} />
                  불참
                </label>
              </div>
              <button type="submit" className="w-full bg-slate-800 text-white font-bold py-3.5 rounded-xl hover:bg-black transition-colors mt-2">투표 완료</button>
            </form>
          </div>
        </div>
      )}
  ...
  const renderRecordsAndRankings = () => {
    ...
    const filteredGames = recordYear === 'all' ? gameResults : gameResults.filter(g => g.date.startsWith(recordYear));
    ...
    const pitcherDataList = players
      ...
      .map(p => getPitcherStats(p, recordType, teamAvgEra))
      .sort((a, b) => {
    ...
    const pitchersByERA = [...players].filter(p => p.primaryRole === '투수' || p.primaryRole === '투타겸업' || parseBaseballInningsToOuts(p.pitching?.innings) > 0).map(p => getPitcherStats(p, 'season', teamAvgEra)).sort((a, b) => parseFloat(a.era) - parseFloat(b.era));
    const pitchersByWins = [...players].filter(p => p.primaryRole === '투수' || p.primaryRole === '투타겸업' || parseBaseballInningsToOuts(p.pitching?.innings) > 0).map(p => getPitcherStats(p, 'season', teamAvgEra)).sort((a, b) => b.wins - a.wins);
    const pitchersByK = [...players].filter(p => p.primaryRole === '투수' || p.primaryRole === '투타겸업' || parseBaseballInningsToOuts(p.pitching?.innings) > 0).map(p => getPitcherStats(p, 'season', teamAvgEra)).sort((a, b) => b.strikeouts - a.strikeouts);
    /* RECORD_RANK_PATCH_V1 */
    ...
    const pitchersByInnings = [...players]
      .filter(p => p.primaryRole === '투수' || p.primaryRole === '투타겸업' || parseBaseballInningsToOuts(p.pitching?.innings) > 0)
      .map(p => getPitcherStats(p, 'season', teamAvgEra))
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex bg-white rounded-xl shadow-sm p-1 border border-gray-200 inline-flex w-full sm:w-auto">
            ...
            <button onClick={() => setRecordType('season')} className={`flex-1 sm:flex-none px-6 py-3 rounded-lg font-bold text-sm transition-all ${recordType === 'season' ? 'bg-slate-800 shadow-sm text-white' : 'text-gray-600 hover:text-gray-900'}`}>시즌 기록 보기</button>
          ...
          {(recordType === 'season' || recordType === 'summary') && (
            <select 
              value={recordYear} 
              onChange={(e) => setRecordYear(e.target.value)} 
              className="px-4 py-2.5 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-slate-800 bg-white font-bold text-gray-700 shadow-sm cursor-pointer"
            >
              <option value="all">전체 누적 기록</option>
              <option value={currentSeasonYear.toString()}>{currentSeasonYear}년 데이터</option>
              <option value={(currentSeasonYear - 1).toString()}>{currentSeasonYear - 1}년 데이터</option>
              <option value={(currentSeasonYear - 2).toString()}>{currentSeasonYear - 2}년 데이터</option>
            </select>
          )}
        </div>
                  ...
                  <tbody className="divide-y divide-gray-100 text-gray-800">
                    {filteredGames.length === 0 ? (
                      ...
                      <tr key={`game-${g.id}`} className="hover:bg-gray-50 cursor-pointer" onClick={() => g.detail && setSelectedGameResult(g)}>
                        ...
                        <td className="p-4 text-center">
                          <span className={`font-black px-3 py-1 rounded-full text-sm flex items-center justify-center mx-auto w-12 ${g.result === '승' ? 'bg-slate-800 text-white' : g.result === '패' ? 'bg-gray-400 text-white' : 'bg-gray-500 text-white'}`}>
                            {g.result}
                          </span>
                        </td>
                        <td className="p-4 text-center">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden overflow-x-auto">
                <table className="w-full text-sm text-center whitespace-nowrap">
                  <thead className="bg-gray-100 border-b border-gray-200 text-gray-700">
                    <tr>
                      <th className="p-3 text-left w-32 sticky left-0 bg-gray-100 z-10 shadow-[1px_0_0_0_#e5e7eb]">이름</th>
                      {['games', 'wins', 'losses', 'saves', 'innings', 'hitsAllowed', 'walksAllowed', 'strikeouts', 'runsAllowed', 'earnedRuns', 'era', 'whip', 'fip'].map(key => {
                        const labels = { games: 'G', wins: 'W', losses: 'L', saves: 'SV', innings: 'IP', hitsAllowed: 'H', walksAllowed: 'BB', strikeouts: 'SO', runsAllowed: 'R', earnedRuns: 'ER', era: 'ERA', whip: 'WHIP', fip: '*FIP' };
                        return (
  ...
  const renderGameResultDetail = () => {
    if (!selectedGameResult || !selectedGameResult.detail) return null;
              ...
              {['summary', 'lineup', 'playbyplay'].map(tab => {
                ...
                return (
                  <button 
                    ...
                    className={`flex-1 sm:flex-none px-4 py-2 rounded-lg font-bold text-sm transition-all ${detailTab === tab ? 'bg-white shadow-sm text-slate-800' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    {labels[tab]}
                  </button>
                )
            ...
            {detailTab === 'summary' && (
              <div className="space-y-6">
                {detail.mvpId && (
                  <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-6 border border-amber-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <h4 className="text-2xl font-black text-gray-800">{String(players.find(p => String(p.id) === String(detail.mvpId))?.name || '알 수 없음')}</h4>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                  ...
                  <div className="overflow-x-auto"><InningBox detail={detail} /></div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                    ...
                    <div className="space-y-3">
                      {detail.statBars?.map(bar => (
                        <div key={`statbar-${bar.label}`} className="grid grid-cols-[30px_1fr_30px] items-center gap-3 text-sm">
                          ...
                          <div className="relative h-6 bg-gray-100 rounded-full overflow-hidden flex items-center justify-center font-bold text-white text-xs">
                            ...
                            <div className="absolute inset-y-0 right-0 bg-slate-700" style={{ width: `${Math.max(5, (bar.right / Math.max(bar.left + bar.right, 1)) * 100)}%` }} />
                            <span className="relative z-10 text-gray-800">{bar.label}</span>
                          </div>
                          <div className="font-black text-slate-700">{bar.right}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                    ...
                    <div className="space-y-3">
                      {(detail.pitchers?.length ? detail.pitchers : [detail.pitcher]).filter(Boolean).map((pitcherRow, idx) => (
                        <div key={`game-pitcher-${idx}`} className="bg-gray-50 rounded-xl p-3 text-sm flex flex-wrap gap-x-4 gap-y-2">
                          ...
                          <div><span className="text-gray-500 text-xs">실점</span> <span className="font-bold">{pitcherRow.runsAllowed || 0}</span></div>
                          <div><span className="text-gray-500 text-xs">자책</span> <span className="font-bold">{pitcherRow.earnedRuns || 0}</span></div>
                          <div><span className="text-gray-500 text-xs">피안타</span> <span className="font-bold">{pitcherRow.hitsAllowed || 0}</span></div>
                          <div><span className="text-gray-500 text-xs">볼넷</span> <span className="font-bold">{pitcherRow.walksAllowed || 0}</span></div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <table className="w-full text-sm min-w-[980px]">
                  <thead className="bg-gray-50 text-gray-600 border-b border-gray-100">
                    <tr>
                      ...
                      {[1,2,3,4,5,6,7,8,9].map(i => <th key={`inning-th-${i}`} className="p-3 text-center">{i}</th>)}
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {detail.lineup?.map((row, idx) => (
                      <tr key={`game-lineup-${row.id || idx}`} className="hover:bg-gray-50">
                        ...
                        {[1,2,3,4,5,6,7,8,9].map(i => (
                          <td key={`inning-td-${i}`} className="p-3 text-center text-xs text-gray-500">{(row.resultByInning?.[`${i}`] || []).join(', ')}</td>
                        ))}
                        <td className="p-3 text-center font-bold bg-gray-50/50">{row.atBats || 0}</td>
                        <td className="p-3 text-center font-bold bg-gray-50/50">{row.hits || 0}</td>
                        <td className="p-3 text-center font-bold bg-gray-50/50">{row.rbi || 0}</td>
                        <td className="p-3 text-center font-bold bg-gray-50/50">{row.runs || 0}</td>
                        <td className="p-3 text-center font-black text-slate-800 bg-slate-100/50">{calculateBattingAverage(row.hits, row.atBats)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                <div className="space-y-3">
                  {detail.playEvents?.map((event, index) => (
                    <div key={`event-${index}`} className="flex gap-4 items-center">
        ...
        {adminSubTab === 'dashboard' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center space-x-4">
              ...
              <div><p className="text-sm text-gray-500 font-medium">등록된 타자</p><h3 className="text-2xl font-bold text-gray-800">{players.filter(p => ['타자','투타겸업'].includes(p.primaryRole)).length}명</h3></div>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center space-x-4">
              <div className="p-3 bg-green-100 text-green-600 rounded-lg"><Activity size={24} /></div>
              <div><p className="text-sm text-gray-500 font-medium">등록된 투수</p><h3 className="text-2xl font-bold text-gray-800">{players.filter(p => ['투수','투타겸업'].includes(p.primaryRole)).length}명</h3></div>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center space-x-4">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 text-gray-600 text-sm border-b border-gray-200">
                    ...
                    <th className="p-4 font-semibold text-right">*FIP</th>
                ...
                <tbody className="divide-y divide-gray-100 text-gray-800">
                  {players.filter(p => ['투수', '투타겸업'].includes(p.primaryRole)).map(p => {
                    const stats = getPitcherStats(p, 'season', teamAvgEra);
                    return (
                      <tr key={`admin-pitcher-${p.id}`} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="p-4 text-right font-semibold text-slate-800">{stats.fip}</td>
                        <td className="p-4 text-right">{stats.games}</td>
            ...
            {!gameState ? (
              ...
              <div>
                <div className="flex justify-between items-center mb-8 border-b pb-4">
                  ...
                  <div className="flex gap-3">
                    {gameState.mode === 'regular_setup' && (
                      <button onClick={downloadLineupCard} className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-3 rounded-xl font-bold transition-colors shadow-sm flex items-center gap-2">
                        <Download size={18} /> 라인업 다운로드
                      </button>
                    )}
                ...
                {gameState.mode === 'regular_setup' && (
                  ...
                  <h4 className="text-lg font-black text-gray-800 mb-4">경기 기본 정보</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-bold text-gray-700 mb-2">경기 날짜</label>
                      <input type="date" value={gameState.gameDate || ''} onChange={(e) => setGameState(prev => ({ ...prev, gameDate: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-slate-800 bg-white" />
                    </div>
                    {gameState.mode === 'regular_setup' && (
                      <>
                        <div>
                              ...
                              <tr key={`lineup-row-${teamKey}-${idx}`} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="p-3">
                                  <select className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-slate-800 bg-white" value={slot.playerId} onChange={(e) => handleLineupChange(teamKey, idx, 'playerId', e.target.value)}>
                                    ...
                                    <optgroup label="타자">{players.filter(p => ['타자','투타겸업','투수'].includes(p.primaryRole)).map(b => <option key={`batter-opt-${b.id}`} value={b.id}>{b.name} (No.{b.uniformNumber})</option>)}</optgroup>
                                  </select>
                                </td>
                                <td className="p-3">
                  ...
                  <div className="flex justify-between items-center bg-black/50 p-4 rounded-2xl mb-12 border border-slate-700">
                    ...
                    <div className="text-center w-1/3 border-x border-slate-700">
                      ...
                      <div className="flex flex-wrap justify-center gap-3 mt-3">
                        <button onClick={() => executeWithHistory(() => forceInningChange())} className="flex items-center justify-center gap-2 min-w-[120px] bg-slate-800 hover:bg-slate-700 border border-slate-600 text-gray-200 text-sm font-semibold px-3 py-2 rounded-2xl transition-colors whitespace-nowrap leading-tight">
                          <FastForward size={14} /> 강제 교대
                        </button>
                        <button onClick={handleUndo} disabled={gameStateHistory.length === 0} className={`flex items-center justify-center gap-2 min-w-[120px] text-sm font-semibold px-3 py-2 rounded-2xl transition-colors whitespace-nowrap leading-tight ${gameStateHistory.length > 0 ? 'bg-amber-600/25 hover:bg-amber-600/45 border border-amber-600/60 text-amber-100' : 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700'}`} title="직전 플레이 취소">
                          <Undo size={14} /> 실행 취소
                        </button>
                        <button onClick={() => openLineupChange(gameState.half === 'top' ? 'teamB' : 'teamA')} className="flex items-center justify-center gap-2 min-w-[140px] bg-emerald-600/25 hover:bg-emerald-600/55 border border-emerald-600/60 text-emerald-100 text-sm font-semibold px-3 py-2 rounded-2xl transition-colors whitespace-nowrap leading-tight" title="수비측 야수 교체">
                          <Users size={14} /> 야수 교체
                        </button>
                  ...
                  <div className="bg-slate-800/80 p-5 rounded-2xl flex justify-between border border-slate-700">
                    ...
                    <div className="flex-1 text-right border-l border-slate-700 pl-4">
                      ...
                      {changingPitcherTeam === (gameState.half === 'top' ? 'teamB' : 'teamA') ? (
                        <select autoFocus className="w-full mt-1 p-1 bg-slate-700 border border-slate-600 text-white rounded outline-none text-sm" onChange={(e) => requestPitcherChange(gameState.half === 'top' ? 'teamB' : 'teamA', e.target.value)} onBlur={() => setChangingPitcherTeam(null)}>
                          <option value="">투수 선택...</option>
                          {players.map(p => <option key={`pitcher-chg-opt-${p.id}`} value={p.id}>{p.name} (No.{p.uniformNumber}) {p.primaryRole !== '투수' ? `· ${p.primaryRole}` : ''}</option>)}
                        </select>
                      ) : (
  ...
  const renderPlayerDetail = () => {
    ...
    const battingStats = getBatterStats(selectedPlayer, 'season');
    const pitchingStats = getPitcherStats(selectedPlayer, 'season', teamAvgEra);
    const careerBattingStats = getBatterStats(selectedPlayer, 'career');
    const careerPitchingStats = getPitcherStats(selectedPlayer, 'career', teamAvgEra);
    const careerBatting = selectedPlayer.batting?.career || {};
    const careerPitching = selectedPlayer.pitching?.career || {};

    const chartData = selectedPlayer.primaryRole === '투수' ? [
      { label: 'ERA', value: Math.max(0, 100 - (parseFloat(pitchingStats.era || 0) * 10)), max: 100 },
      { label: 'K', value: selectedPlayer.pitching?.strikeouts || 0, max: 100 },
      { label: 'W', value: (selectedPlayer.pitching?.wins || 0) * 10, max: 100 },
      { label: 'IP', value: parseFloat(selectedPlayer.pitching?.innings || 0), max: 100 },
      { label: 'WHIP', value: Math.max(0, 100 - (parseFloat(pitchingStats.whip || 0) * 30)), max: 100 }
    ] : [
      { label: 'Contact', value: parseFloat(battingStats.avg || 0) * 300, max: 100 },
      { label: 'Power', value: parseFloat(battingStats.slg || 0) * 200, max: 100 },
      { label: 'Eye', value: parseFloat(battingStats.obp || 0) * 250, max: 100 },
      { label: 'Run', value: (selectedPlayer.batting?.steals || 0) * 5, max: 100 },
      { label: 'RBI', value: selectedPlayer.batting?.rbi || 0, max: 100 }
    ];
        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden max-h-[90vh] flex flex-col relative" onClick={e => e.stopPropagation()}>
          <div className="bg-gradient-to-r from-slate-800 to-slate-900 text-white p-8 flex items-center gap-6 relative">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-gray-300 font-bold text-sm tracking-widest mb-1">POLARIS · {String(selectedPlayer.position)}</p>
            ...
            <div className="hidden md:flex flex-col items-center justify-center bg-white/5 rounded-xl p-4 ml-auto w-48 shrink-0">
              <RadarChart data={chartData} color="#38bdf8" />
            </div>
          </div>

          <div className="overflow-y-auto p-8 space-y-8">
            <div className="flex md:hidden flex-col items-center justify-center bg-gray-50 rounded-xl p-4 w-full">
              <h4 className="text-sm font-bold text-gray-500 mb-2">시즌 능력치 분석</h4>
              <RadarChart data={chartData} color="#38bdf8" />
            </div>

            {['타자', '투타겸업'].includes(selectedPlayer.primaryRole) && (
              <div>
                <h3 className="text-lg font-black text-gray-800 mb-3 flex items-center gap-2"><span className="w-1 h-5 bg-slate-800 rounded"></span> 타격 기록 ({seasonLabel})</h3>
            ...
            {['투수', '투타겸업'].includes(selectedPlayer.primaryRole) && (
              <div>
                <h3 className="text-lg font-black text-gray-800 mb-3 flex items-center gap-2"><span className="w-1 h-5 bg-emerald-600 rounded"></span> 투구 기록 ({seasonLabel})</h3>
                <div className="overflow-x-auto bg-gray-50/50 rounded-xl border border-gray-200">
                  <table className="w-full text-sm">
                    <thead className="text-gray-600 border-b border-gray-200">
                      <tr><th className="p-3 font-semibold">G</th><th className="p-3 font-semibold">W</th><th className="p-3 font-semibold">L</th><th className="p-3 font-semibold">SV</th><th className="p-3 font-semibold">IP</th><th className="p-3 font-semibold">H</th><th className="p-3 font-semibold">BB</th><th className="p-3 font-semibold">SO</th><th className="p-3 font-semibold">ER</th><th className="p-3 font-semibold text-gray-800">ERA</th><th className="p-3 font-semibold text-gray-800">WHIP</th><th className="p-3 font-semibold text-gray-800">*FIP</th></tr>
                    </thead>
                    <tbody className="text-gray-800 font-bold text-center">
                      <tr>
                        <td className="p-3">{selectedPlayer.pitching?.games || 0}</td><td className="p-3">{selectedPlayer.pitching?.wins || 0}</td><td className="p-3">{selectedPlayer.pitching?.losses || 0}</td><td className="p-3">{selectedPlayer.pitching?.saves || 0}</td><td className="p-3">{selectedPlayer.pitching?.innings || 0}</td><td className="p-3">{selectedPlayer.pitching?.hitsAllowed || 0}</td><td className="p-3">{selectedPlayer.pitching?.walksAllowed || 0}</td><td className="p-3">{selectedPlayer.pitching?.strikeouts || 0}</td><td className="p-3">{selectedPlayer.pitching?.earnedRuns || 0}</td><td className="p-3 text-gray-900 text-lg">{String(pitchingStats.era)}</td><td className="p-3 text-gray-900 text-lg">{String(pitchingStats.whip)}</td><td className="p-3 text-gray-900 text-lg">{String(pitchingStats.fip)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            {['타자', '투타겸업'].includes(selectedPlayer.primaryRole) && (
              <div>
                <h3 className="text-lg font-black text-gray-800 mb-3 flex items-center gap-2">
                  <span className="w-1 h-5 bg-amber-500 rounded"></span> 통산 타격 기록
                </h3>
                <div className="overflow-x-auto bg-amber-50/30 rounded-xl border border-amber-200">
                  <table className="w-full text-sm">
                    <thead className="text-gray-600 border-b border-amber-200">
                      <tr>
                        <th className="p-3 font-semibold">G</th>
                        <th className="p-3 font-semibold">AB</th>
                        <th className="p-3 font-semibold">R</th>
                        <th className="p-3 font-semibold">H</th>
                        <th className="p-3 font-semibold">2B</th>
                        <th className="p-3 font-semibold">3B</th>
                        <th className="p-3 font-semibold">HR</th>
                        <th className="p-3 font-semibold">RBI</th>
                        <th className="p-3 font-semibold">BB</th>
                        <th className="p-3 font-semibold">SO</th>
                        <th className="p-3 font-semibold">SB</th>
                        <th className="p-3 font-semibold text-gray-800">AVG</th>
                        <th className="p-3 font-semibold text-gray-800">OPS</th>
                      </tr>
                    </thead>
                    <tbody className="text-gray-800 font-bold text-center">
                      <tr>
                        <td className="p-3">{careerBatting.games || 0}</td>
                        <td className="p-3">{careerBatting.atBats || 0}</td>
                        <td className="p-3">{careerBatting.runs || 0}</td>
                        <td className="p-3">{careerBatting.hits || 0}</td>
                        <td className="p-3">{careerBatting.doubles || 0}</td>
                        <td className="p-3">{careerBatting.triples || 0}</td>
                        <td className="p-3">{careerBatting.homeRuns || 0}</td>
                        <td className="p-3">{careerBatting.rbi || 0}</td>
                        <td className="p-3">{careerBatting.walks || 0}</td>
                        <td className="p-3">{careerBatting.strikeouts || 0}</td>
                        <td className="p-3">{careerBatting.steals || 0}</td>
                        <td className="p-3 text-amber-700 text-lg">{String(careerBattingStats.avg || '0.000')}</td>
                        <td className="p-3 text-amber-700 text-lg">{String(careerBattingStats.ops || '0.000')}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            {['투수', '투타겸업'].includes(selectedPlayer.primaryRole) && (
              <div>
                <h3 className="text-lg font-black text-gray-800 mb-3 flex items-center gap-2">
                  <span className="w-1 h-5 bg-amber-500 rounded"></span> 통산 투구 기록
                </h3>
                <div className="overflow-x-auto bg-amber-50/30 rounded-xl border border-amber-200">
                  <table className="w-full text-sm">
                    <thead className="text-gray-600 border-b border-amber-200">
                      <tr>
                        ...
                        <th className="p-3 font-semibold">BB</th>
                        <th className="p-3 font-semibold">SO</th>
                        <th className="p-3 font-semibold">ER</th>
                        <th className="p-3 font-semibold text-gray-800">ERA</th>
                        <th className="p-3 font-semibold text-gray-800">WHIP</th>
                        <th className="p-3 font-semibold text-gray-800">*FIP</th>
                      </tr>
                    </thead>
                    <tbody className="text-gray-800 font-bold text-center">
                      <tr>
                        <td className="p-3">{careerPitching.games || 0}</td>
                        <td className="p-3">{careerPitching.wins || 0}</td>
                        <td className="p-3">{careerPitching.losses || 0}</td>
                        <td className="p-3">{careerPitching.saves || 0}</td>
                        <td className="p-3">{careerPitching.innings || 0}</td>
                        <td className="p-3">{careerPitching.hitsAllowed || 0}</td>
                        <td className="p-3">{careerPitching.walksAllowed || 0}</td>
                        <td className="p-3">{careerPitching.strikeouts || 0}</td>
                        <td className="p-3">{careerPitching.earnedRuns || 0}</td>
                        <td className="p-3 text-amber-700 text-lg">{String(careerPitchingStats.era || '0.00')}</td>
                        <td className="p-3 text-amber-700 text-lg">{String(careerPitchingStats.whip || '0.00')}</td>
                        <td className="p-3 text-amber-700 text-lg">{String(careerPitchingStats.fip || '0.00')}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900" style={{ fontFamily: "'Pretendard', sans-serif" }}>
      ...
      <nav className="fixed top-0 left-0 w-full h-14 bg-black/60 backdrop-blur-md text-white z-50 border-b border-white/10 shadow-sm" style={{ fontFamily: "'Malgun Gothic', '맑은 고딕', sans-serif" }}>
        <div className="max-w-[1600px] mx-auto px-6 h-full flex items-center justify-between">
          ...
          <div className="hidden sm:flex space-x-6 md:space-x-10 text-sm md:text-base font-black tracking-widest">
            <button onClick={() => setActiveTab('schedule')} className={`hover:text-gray-300 transition-colors ${activeTab === 'schedule' ? 'text-white' : ''}`}>일정/참석</button>
      ...
      {lineupChangeTeam && gameState && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[85] p-4" onClick={() => { setLineupChangeTeam(null); setLineupDraft([]); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-gray-200 bg-gray-50">
              <div>
                <h3 className="text-xl font-black text-gray-800">라인업 / 야수 교체</h3>
                <p className="text-sm text-gray-500 mt-1">{gameState[lineupChangeTeam]?.name}</p>
              </div>
              <button onClick={() => { setLineupChangeTeam(null); setLineupDraft([]); }} className="text-gray-400 hover:text-gray-600">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 p-6">
              <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 mb-4 text-xs text-amber-800">
                ⚾ 선수를 변경하면 새 선수의 게임 기록은 0부터 시작하며, 교체되어 나간 선수의 기록은 보존됩니다.
              </div>
              <div className="overflow-x-auto bg-gray-50 rounded-xl border border-gray-200">
                <table className="w-full text-sm text-left">
                  <thead className="bg-gray-100 text-gray-600 font-semibold border-b border-gray-200">
                    <tr>
                      <th className="p-3 text-center w-20">타순</th>
                      <th className="p-3">선수</th>
                      <th className="p-3 w-40">수비 위치</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {lineupDraft.map((slot, idx) => {
                      const orig = gameState[lineupChangeTeam].lineup[idx];
                      const changed = orig && String(orig.id) !== String(slot.newPlayerId);
                      return (
                        <tr key={`lc-row-${idx}`} className={changed ? 'bg-amber-50' : 'hover:bg-white'}>
                          <td className="p-3 text-center font-bold text-gray-700">{idx + 1}번</td>
                          <td className="p-3">
                            <select
                              className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-slate-800 bg-white"
                          ...
                          <td className="p-3">
                            <select
                              className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-slate-800 bg-white"
                              value={slot.assignedPosition}
                              onChange={(e) => updateLineupDraft(idx, 'assignedPosition', e.target.value)}
                            >
                              <option value="">포지션...</option>
                              {POSITIONS.map((pos) => (
                                <option key={`lc-pos-${idx}-${pos}`} value={pos}>{pos}</option>
                              ))}
                            </select>
                          </td>
                        </tr>
            ...
            <div className="flex gap-2 p-6 border-t border-gray-100 bg-gray-50">
              <button onClick={() => { setLineupChangeTeam(null); setLineupDraft([]); }} className="flex-1 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl transition-colors">
                취소
              </button>
              <button onClick={saveLineupChange} className="flex-1 px-4 py-3 bg-slate-800 hover:bg-black text-white font-bold rounded-xl transition-colors">
                교체 적용
              </button>
            </div>
          </div>
      ...
      {pendingSac && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[80] p-4" onClick={() => setPendingSac(null)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <h3 className="text-xl font-black text-gray-800 mb-3">{pendingSac.actionLabel} - 주자 처리</h3>
            <p className="text-sm text-gray-600 mb-4">타자는 아웃됩니다. 각 주자를 어떻게 처리할까요?</p>
            <div className="space-y-3">
              {pendingSac.runners.map((r, idx) => (
                <div key={`sac-runner-${idx}`} className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                  <div className="font-bold text-gray-800 mb-2">
                    <span className="inline-block bg-yellow-400 text-black text-xs font-black px-2 py-0.5 rounded mr-2">
                      {r.base + 1}B
                    </span>
                  ...
                  <select
                    value={r.choice}
                    onChange={(e) => {
                      const next = [...pendingSac.runners];
                      next[idx] = { ...next[idx], choice: Number(e.target.value) };
                      setPendingSac({ ...pendingSac, runners: next });
                    }}
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button
                onClick={() => {
                  const { actionLabel, runners } = pendingSac;
                  setPendingSac(null);
                  executeSacResolution(actionLabel, runners);
                }}
                className="flex-1 bg-slate-800 hover:bg-black text-white font-bold py-3 rounded-xl transition-colors"
              >
                확인
      ...
      {pendingGroundOut && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[80] p-4" onClick={() => setPendingGroundOut(null)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              {pendingGroundOut.runners.map((r, idx) => (
                <div key={`ground-runner-${idx}`} className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                  ...
                  <select
                    value={r.choice}
                    onChange={(e) => {
                      const next = [...pendingGroundOut.runners];
                      next[idx] = { ...next[idx], choice: Number(e.target.value) };
                      setPendingGroundOut({ ...pendingGroundOut, runners: next });
                    }}
                    className="w-full p-2.5 border border-gray-300 rounded-lg bg-white font-medium outline-none focus:ring-2 focus:ring-slate-800"
                  >
                    {getRunnerAdvanceOptions(r.base, 1).map(opt => (
            ...
            <div className="flex gap-2 mt-5">
              <button
                onClick={() => setPendingGroundOut(null)}
                className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 rounded-xl transition-colors"
              >
                취소
              </button>
      ...
      {pitcherSwapAsk && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[80] p-4" onClick={() => setPitcherSwapAsk(null)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <h3 className="text-xl font-black text-gray-800 mb-3">지명타자 처리</h3>
            <p className="text-sm text-gray-600 mb-5">
              라인업에 이미 지명타자가 있습니다. 야수를 투수로 올리면 일반적으로 DH가 소멸됩니다. 어떻게 처리할까요?
            </p>
            <div className="space-y-3">
              <button
                onClick={() => {
              ...
              <button
                onClick={() => {
                  const { teamKey, newPitcherId } = pitcherSwapAsk;
                  setPitcherSwapAsk(null);
                  executePitcherChange(teamKey, newPitcherId);
                }}
                className="w-full bg-slate-800 hover:bg-black text-white font-bold py-3 rounded-xl"
              >
                DH 유지 (투수는 타순에서 제외)
              </button>
              <button
                onClick={() => setPitcherSwapAsk(null)}
